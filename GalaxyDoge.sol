...
contract GalaxyDoge is Context, IERC20, Ownable {
  ...
  string private _name = 'Galaxy Doge';
  string private _symbol = 'GALAXYDOGE';
 ...
 uint256 public reflectFeeSell = 4;
  ...
  address private _marketingWalletAddress = 0x22292407144f7Ca77bdEff34C2DF140db0B6f464; //Affixed and created dynamically by the smart contract on construction time.
  address public _buybackWallet = 0xDA46651A52F6677973F5EA44185fEbf4a7155Cf2;
  ...
  address public immutable GalaxyDogeUniswapV2Pair;
  mapping(address => bool) _isExcludedFromMaxWalletLimit;

  bool inSwapAndLiquify;
  bool public swapAndLiquifyEnabled = true;

  uint256 public _maxTxAmount = 4000000000000000 * 10**9 * 10**9; //1% (0.005x) of Total circulating supply, after burn
  uint256 private numTokensSellToAddToLiquidity = 400000000 * 10**9; //1 order of magnitude smaller than the max tx amount (so, div by 10, sort of)
  ...
  constructor() {
      address _newOwner = 0x3ECeF953e786c743E57C22F54380b2b704966040;
    ...
    GalaxyDogeUniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
 ...
 function setSellFees() private {
  ...
  function _transfer(
    ...
    if ((!launched() && to == GalaxyDogeUniswapV2Pair) || manualLaunch) {
      require(
        balanceOf(from) > 0,
    ...
    if(!_isExcludedFromMaxWalletLimit[from] && !_isExcludedFromMaxWalletLimit[to] && to != GalaxyDogeUniswapV2Pair) {
  ...
  function _tokenTransfer(
    ...
    if(takeFee && sender == GalaxyDogeUniswapV2Pair) {
        // buy
        setBuyFees();
    } else if(takeFee && recipient == GalaxyDogeUniswapV2Pair) {
        // sell
        setSellFees();
    }
    ...
    if(takeFee && (sender == GalaxyDogeUniswapV2Pair || recipient == GalaxyDogeUniswapV2Pair)) {
        restoreAllFee();
    }
